	...
	$window.on('load', function () {
		...
		if ($(".preloader-close").length) {
			$(".preloader-close").on("click", function () {
				$('.loader-wrap').delay(200).fadeOut(1500);
			})
		}
		$(window).on('load', function () {
			handlePreloader();
...
function initializeCounters(counters) {
    ...
    counters.forEach(node => {
        const initialValue = parseInt(node.innerText.replace(/\./g, ''), 10);
		
        const counter = new bCounter({
            node: node,
            duration: 10000,
            autorun: false,
            startValue: initialValue,
            onUpdate: amount => {
                node.innerText = formatNumber(amount);
            },
...
initializeCounters(document.querySelectorAll('[data-counter]'));
